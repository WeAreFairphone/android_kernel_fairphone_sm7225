...
* Static variables
*****************************************************************************/
#define FTS_FW_REQUEST_SUPPORT                      0 // MODIFIED by hongwei.tian, 2019-11-27,BUG-8596641
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
    ...
    /* MODIFIED-BEGIN by hongwei.tian, 2020-05-21,BUG-9467258*/
    if (ts_data->pdata->id0 == 0 && ts_data->pdata->id1 == 0)
    {
        FTS_INFO("load TP fw for TDT lcm");
        info = &module_list[1];
    }
    else
    {
        FTS_INFO("load TP fw fordefault lcm");
        info = &module_list[0];
    }
    /* MODIFIED-END by hongwei.tian,BUG-9467258*/
