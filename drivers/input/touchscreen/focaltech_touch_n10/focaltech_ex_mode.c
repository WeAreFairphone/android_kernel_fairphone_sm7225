...
static DEVICE_ATTR(glove_mode, S_IRUGO | S_IWUSR,
                   fts_glove_mode_show, fts_glove_mode_store);
...
int fts_ex_mode_init(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->tct_touch_dev) {
        ret = device_create_file(ts_data->tct_touch_dev, &dev_attr_glove_mode);
        if (ret < 0)
            FTS_DEBUG("Failed to create file glove_mode!");
    } else
        FTS_DEBUG("no tct_touch_dev!");
