...
/* MODIFIED-BEGIN by hongwei.tian, 2020-02-26,BUG-8956218*/
static int fts_get_module_id(struct fts_ts_data *data)
{
    int ret = 0;

    FTS_FUNC_ENTER();
    /* request id0 gpio */
    if (gpio_is_valid(data->pdata->id0_gpio)) {
        ret = gpio_request(data->pdata->id0_gpio, "fts_id0_gpio");
        if (ret) {
            FTS_ERROR("[GPIO]id0 gpio request failed");
            goto err_id0_gpio_req;
        }
        ret = gpio_direction_input(data->pdata->id0_gpio);
        if (ret) {
            FTS_ERROR("[GPIO]set_direction for id0 gpio failed");
            goto err_id0_gpio_dir;
        }
        msleep(1);
        data->pdata->id0 = gpio_get_value(data->pdata->id0_gpio);
    }

    /* request id1 gpio */
    if (gpio_is_valid(data->pdata->id1_gpio)) {
        ret = gpio_request(data->pdata->id1_gpio, "fts_id1_gpio");
        if (ret) {
            FTS_ERROR("[GPIO]id1 gpio request failed");
            goto err_id0_gpio_dir;
        }
        ret = gpio_direction_input(data->pdata->id1_gpio);
        if (ret) {
            FTS_ERROR("[GPIO]set_direction for id1 gpio failed");
            goto err_id1_gpio_dir;
        }
        msleep(1);
        data->pdata->id1 = gpio_get_value(data->pdata->id1_gpio);
    }
    if (data->pdata->id0 == 0 && data->pdata->id1 == 0)
        data->pdata->panel_vendor = 0x00; /*tdt2nd*/
    else if (data->pdata->id0 == 1 && data->pdata->id1 == 0)
        data->pdata->panel_vendor = 0x10; /*csot1st*/
    else
        data->pdata->panel_vendor = 0x10;

    if (gpio_is_valid(data->pdata->id0_gpio))
        gpio_free(data->pdata->id0_gpio);
    if (gpio_is_valid(data->pdata->id1_gpio))
        gpio_free(data->pdata->id1_gpio);

    FTS_INFO("id0_gpio=%d, id1_gpio=%d, id0=%d, id1=%d",
            data->pdata->id0_gpio, data->pdata->id1_gpio, data->pdata->id0, data->pdata->id1);
    FTS_FUNC_EXIT();
    return 0;
...
/* MODIFIED-END by hongwei.tian,BUG-8956218*/
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    /* MODIFIED-BEGIN by hongwei.tian, 2020-02-26,BUG-8956218*/
    pdata->id0_gpio = of_get_named_gpio_flags(np, "focaltech,id0-gpio",
                      0, &pdata->id0_gpio_flags);
    if (pdata->id0_gpio < 0)
        FTS_ERROR("Unable to get id0_gpio");

    pdata->id1_gpio = of_get_named_gpio_flags(np, "focaltech,id1-gpio",
                      0, &pdata->id1_gpio_flags);
    if (pdata->id1_gpio < 0)
        FTS_ERROR("Unable to get id1_gpio");
        /* MODIFIED-END by hongwei.tian,BUG-8956218*/
...
static void tct_touch_class_create(struct fts_ts_data *data)
{
    if (data->tct_touch_class == NULL)
        data->tct_touch_class = class_create(THIS_MODULE, "tct_touch");

    if (!data->tct_touch_class)
        FTS_ERROR("failed to create tct_touch_class!");

    if (data->tct_touch_class) {
        data->tct_touch_dev =
            device_create(data->tct_touch_class, NULL, 0, NULL, "tct_touch_dev");
        if (!data->tct_touch_dev)
            FTS_DEBUG("Failed to create tct_touch_dev!");
    }
}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    /* MODIFIED-BEGIN by hongwei.tian, 2020-02-26,BUG-8956218*/
    ret = fts_get_module_id(ts_data);
    if (ret)
        FTS_ERROR("get modle id");
        /* MODIFIED-END by hongwei.tian,BUG-8956218*/
    ...
    tct_touch_class_create(ts_data);
    ...
    if (active_panel) {
        ret = drm_panel_notifier_register(active_panel,
            &ts_data->fb_notif);
        if (ret < 0)
            FTS_ERROR("Failed to register fb notifier client");
    }
#elif defined(CONFIG_FB)
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    if (active_panel) {
		ret = drm_panel_notifier_unregister(active_panel,
				&ts_data->fb_notif);
		if (ret < 0)
            FTS_ERROR("Failed to unregister fb notifier client");
	}
#elif defined(CONFIG_FB)
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
    FTS_ERROR("power suspend enter!"); // MODIFIED by hongwei.tian, 2020-04-10,BUG-8979194
    ...
    ts_data->suspended = true;
    FTS_ERROR("power suspend !"); // MODIFIED by hongwei.tian, 2020-04-10,BUG-8979194
    FTS_FUNC_EXIT();
    return 0;
}
...
static int fts_ts_resume(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
    FTS_ERROR("power resumed enter !"); // MODIFIED by hongwei.tian, 2020-04-10,BUG-8979194
    if (!ts_data->suspended) {
    ...
    ts_data->suspended = false;
    FTS_ERROR("power resumed !"); // MODIFIED by hongwei.tian, 2020-04-10,BUG-8979194
    FTS_FUNC_EXIT();
    return 0;
}
...
int fts_esd_check(void)
{
    ...
    if (ret < 0)
        pr_info("%s i2c read failed, ret:%d", __func__, ret);
	else
		pr_info("%s i2c read sucess, reg_value:%d", __func__, reg_value);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (check_dt(dp)) {
		if (!check_default_tp(dp, "qcom,i2c-touch-active"))
			...
			error = -ENODEV;
        FTS_ERROR("check_dt failed, error=%d", error);
		return error;
	}
