...
*****************************************************************************/
u8 pb_file_ft8716[] = {
#include "../include/pramboot/FT8716_Pramboot_V0.5_20160723.i"
};
...
static int fts_ft8716_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
    int ret = 0;
    u32 start_addr = 0;
    u8 cmd[4] = { 0 };
    ...
    if (FLASH_MODE_PARAM == mode) {
        ...
        start_addr += upgrade_func_ft8716.paramcfgoff;
    }
...
static int fts_ft8716_upgrade(u8 *buf, u32 len)
{
    ...
    app_len = len;
    tmpbuf = buf;
    ret = fts_ft8716_upgrade_mode(FLASH_MODE_APP, tmpbuf, app_len);
    if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
static int fts_ft8716_param_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 param_length = 0;
    ...
    tmpbuf = buf + upgrade_func_ft8716.paramcfgoff;
    param_length = len - upgrade_func_ft8716.paramcfgoff;
    ret = fts_ft8716_upgrade_mode(FLASH_MODE_PARAM, tmpbuf, param_length);
    if (ret < 0) {
        FTS_INFO("fw upgrade fail,reset to normal boot");
        if (fts_fwupg_reset_in_boot() < 0) {
            FTS_ERROR("reset to normal boot fail");
...
struct upgrade_func upgrade_func_ft8716 = {
    ...
    .param_upgrade = fts_ft8716_param_upgrade,
};
